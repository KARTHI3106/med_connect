...
export const patientApi = {
  ...
  revokeConsent: async (patientId: string, consentId: string) => {
    const response = await api.delete(
      `/patient/${patientId}/consents/${consentId}`,
    );
    return response.data;
  },
  ...
  searchPharmacies: async (query: string) => {
    const response = await api.get("/patient/search/pharmacies", {
      params: { q: query },
    });
    return response.data;
  },
...
export const doctorApi = {
  ...
  searchPatients: async (doctorId: string, query: string) => {
    const response = await api.get(`/doctor/${doctorId}/search-patients`, {
      params: { query },
    });
    return response.data;
  },
...
export const pharmacyApi = {
  ...
  getPatientPrescriptions: async (patientId: string) => {
    const response = await api.get(
      `/pharmacy/patient/${patientId}/prescriptions`,
    );
    return response.data;
  },
