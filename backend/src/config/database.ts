...
export const inMemoryDb = {
  ...
  vitalReadings: new Map<string, any>(),
  baselines: new Map<string, any>(),
  riskAssessments: new Map<string, any>(),
  alertEvents: new Map<string, any>(),
  healthScores: new Map<string, any>(),
  isDemo: true,
};
...
export async function initializeDatabase() {
  ...
  if (!pool && !useInMemory) {
    try {
      pool = mysql.createPool({
        host:
          process.env.TIDB_HOST || "gateway01.us-west-2.prod.aws.tidbcloud.com",
        port: parseInt(process.env.TIDB_PORT || "4000"),
      ...
      console.warn(
        "⚠️ Could not create connection pool, switching to in-memory",
      );
      useInMemory = true;
    }
  }
  ...
  try {
    ...
    try {
      await connection.execute(
        "ALTER TABLE prescriptions ADD COLUMN prescription_hash VARCHAR(66)",
      );
    } catch (e) {
      // Ignore error if column already exists
    }
...
function seedInMemoryData() {
  ...
  inMemoryDb.users.set("user-patient-aadhi", {
    ...
    name: "Aadhi",
    email: "aadhi.patient@example.com",
    phone: "+919150590195",
    role: "patient",
  ...
  inMemoryDb.users.set("user-doctor-aadhi", {
    ...
    name: "Dr. Aadhi Care",
    email: "dr.aadhi.care@example.com",
    phone: "+919790830025",
    role: "doctor",
