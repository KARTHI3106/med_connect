...
async function resolvePharmacyId(userId: string, pool: any): Promise<string> {
  if (pool) {
    ...
    const ph = Array.from(inMemoryDb.pharmacies.values()).find(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (pool) {
        ...
        const now = new Date();
        const consent = Array.from(inMemoryDb.consents.values()).find(
          (c) =>
    ...
    try {
      ...
      if (qrData && !prescriptionId) {
        try {
          ...
          return res
            .status(400)
            .json({ success: false, error: "Invalid QR data" });
        }
      }
      ...
      if (!searchId) {
        return res
          .status(400)
          .json({ success: false, error: "Prescription ID or QR required" });
      }
      ...
      if (pool) {
        ...
        const isValid =
          rx.status === "active" && new Date(rx.valid_until) > new Date();
        ...
        return res.json({
          ...
          data: {
            ...
            blockchainHash: rx.blockchain_hash,
            isExpired: new Date(rx.valid_until) <= new Date(),
            isDispensed: rx.status === "dispensed",
          },
        });
      } else {
        ...
        const doctor = Array.from(inMemoryDb.doctors.values()).find(
          (d) => d.id === rx.doctor_id,
        );
        const patient = Array.from(inMemoryDb.patients.values()).find(
          (p) => p.id === rx.patient_id,
        );
        const isValid =
          rx.status === "active" && new Date(rx.valid_until) > new Date();
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      return res
        .status(500)
        .json({ success: false, error: "Failed to get prescriptions" });
    }
  },
);
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      let prescriptionHash: string | null = null;

      if (pool) {
        const [rows] = (await pool.execute(
          "SELECT id, prescription_hash, blockchain_hash FROM prescriptions WHERE id = ? AND patient_id = ? LIMIT 1",
        ...
        prescriptionHash =
          rows[0].prescription_hash || rows[0].blockchain_hash || null;

        // Backward-compat: old rows may not have prescription_hash yet
        if (!rows[0].prescription_hash && prescriptionHash) {
          try {
            await pool.execute(
              "UPDATE prescriptions SET prescription_hash = ? WHERE id = ? AND prescription_hash IS NULL",
              [prescriptionHash, prescriptionId],
            );
          } catch {
            // Non-fatal
          }
        ...
        const rx = inMemoryDb.prescriptions.get(prescriptionId);
        if (!rx || rx.patient_id !== patientId) {
          return res
            .status(404)
            .json({ success: false, error: "Prescription not found" });
        }
        prescriptionHash = rx.prescription_hash || rx.blockchain_hash || null;
      }
      ...
      if (!prescriptionHash) {
        return res.status(400).json({
          success: false,
          error:
            "Prescription hash missing, re-issue prescription or migrate data",
        });
      }
      ...
      return res.status(500).json({ success: false, error: "Dispense failed" });
    }
  },
);
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      return res.json({
        ...
        summary: {
          ...
          expiringSoon: inventory.filter(
            (i) => i.expiryWarning === "EXPIRING_SOON",
          ).length,
          expired: inventory.filter((i) => i.expiryWarning === "EXPIRED")
            .length,
      ...
      return res
        .status(500)
        .json({ success: false, error: "Failed to get inventory" });
    }
  },
);
    ...
    try {
      ...
      if (!medicineName || !batchNumber || !quantity || !expiryDate) {
        return res.status(400).json({
          success: false,
          error:
            "Missing required fields: medicineName, batchNumber, quantity, expiryDate",
        });
      }
      ...
      return res
        .status(500)
        .json({ success: false, error: "Failed to add inventory" });
    }
  },
);
