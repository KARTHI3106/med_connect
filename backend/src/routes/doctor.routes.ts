...
async function resolveDoctorId(userId: string, pool: any): Promise<string> {
  if (pool) {
    ...
    const d = Array.from(inMemoryDb.doctors.values()).find(
...
router.get("/:doctorId/patients", async (req: Request, res: Response) => {
  try {
    ...
    if (pool) {
      ...
      const now = new Date();
      const consentsForDoctor = Array.from(inMemoryDb.consents.values()).filter(
        (c) =>
      ...
      const patientIds = [
        ...new Set(consentsForDoctor.map((c) => c.patient_id)),
      ];
    ...
    return res
      .status(500)
      .json({ success: false, error: "Failed to get patients" });
  }
});
...
router.post("/:doctorId/prescribe", async (req: Request, res: Response) => {
  try {
    ...
    if (pool) {
      ...
      const now = new Date();
      hasConsent = Array.from(inMemoryDb.consents.values()).some(
        (c) =>
          ...
          c.is_active &&
          new Date(c.expires_at) > now &&
          (Array.isArray(c.consent_types)
            ? c.consent_types.includes("write")
            : c.consent_type === "write"),
      );
    }
    ...
    if (pool) {
      await pool.execute(
        `
                INSERT INTO prescriptions (id, patient_id, doctor_id, diagnosis, medicines, prescription_hash, blockchain_hash, prescription_date, valid_until, notes)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            `,
        [
          id,
    ...
    return res
      .status(500)
      .json({ success: false, error: "Failed to issue prescription" });
  }
});

// Search patients by name or ID
router.get(
  "/:doctorId/search-patients",
      ...
      if (pool) {
        ...
        const patients = Array.from(inMemoryDb.patients.values())
          ...
          .map((p) => {
            ...
            const hasConsent = Array.from(inMemoryDb.consents.values()).some(
              (c) =>
                c.patient_id === p.id &&
                c.is_active &&
...
router.get(
  "/:doctorId/patient/:patientId/history",
  async (req: Request, res: Response) => {
    try {
      ...
      if (!hasConsent) {
        return res
          ...
          .json({
            success: false,
            error: "No valid read consent from patient",
          });
      }
      ...
      if (pool) {
        ...
        const prescriptions = Array.from(inMemoryDb.prescriptions.values())
          ...
          .sort(
            (a, b) =>
              new Date(b.prescription_date || 0).getTime() -
              new Date(a.prescription_date || 0).getTime(),
          )
          .slice(0, 50);
      ...
      return res
        .status(500)
        .json({ success: false, error: "Failed to get patient history" });
    }
  },
);
...
function checkDrugInteractions(medicines: any[]): {
  hasInteraction: boolean;
  warnings: string[];
} {
  const warnings: string[] = [];
  const medicineNames = medicines.map((m) =>
    (m.name || m.medicineName || "").toLowerCase(),
  );
...
function checkAllergies(
  medicines: any[],
  allergies: string[],
): { hasAllergy: boolean; warnings: string[] } {
  const warnings: string[] = [];
  ...
  for (const med of medicines) {
    ...
    for (const allergy of allergyList) {
      if (
        ...
        warnings.push(
          `🚨 ALLERGY: Patient allergic to ${allergy}, prescribed ${medName}`,
        );
      }
    }
  }
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (pool) {
        const [patients] = (await pool.execute(
          "SELECT allergies, chronic_conditions FROM patients WHERE id = ?",
          [patientId],
        )) as any;
      ...
      if (safetyResult.attestationHash) {
        await blockchainService.recordSafetyAttestation(
          blockchainHash,
          safetyResult.attestationHash,
        );
      }
