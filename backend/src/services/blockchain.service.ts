...
// MedConnect ABI (must match blockchain/contracts/MedConnect.sol)
const MEDCONNECT_ABI = [
  ...
  "function verifyPrescription(bytes32 prescriptionHash) external view returns (bool exists, uint256 timestamp, address patient, address doctor, bool isDispensed)",
  "function verifyDispense(bytes32 prescriptionHash) external view returns (bool isDispensed, address dispensedBy, uint256 dispensedAt)",
  "function registerBatch(bytes32 batchHash) external",
  ...
  "function recordSafetyAttestation(bytes32 prescriptionHash, bytes32 attestationHash) external",
  "function verifySafetyAttestation(bytes32 prescriptionHash) external view returns (bool exists, bytes32 attestationHash)",
  ...
  "event SafetyAttestationRecorded(bytes32 indexed prescriptionHash, bytes32 attestationHash, uint256 timestamp)",
];
...
export class BlockchainService {
    ...
    try {
      ...
      if (process.env.PRIVATE_KEY) {
        ...
        if (isAddressValid) {
          ...
          console.log(
            `✅ Blockchain contract initialized at: ${process.env.CONTRACT_ADDRESS}`,
          );
        } else {
          console.warn(
            `⚠️ CONTRACT_ADDRESS is missing or invalid in .env (falling back to demo mode)`,
          );
        }
      }
      ...
      console.warn(
        "⚠️ Blockchain service initialization failed (demo mode)",
        error,
      );
    }
  }
  ...
  async recordDispense(
    prescriptionHash: string,
    _pharmacyAddress: string = ethers.ZeroAddress,
  ): Promise<{ success: boolean; txHash?: string; error?: string }> {
    if (!this.contract || !this.wallet) {
      const simulatedTxHash = `0x${Buffer.from(
        Date.now().toString() + Math.random().toString(),
    ...
    try {
      const tx = await this.contract.recordDispense(prescriptionHash);
      const receipt = await tx.wait();
      return { success: true, txHash: receipt.hash };
    } catch (error: any) {
      return { success: false, error: error.message };
      ...
      if (data.name && data.manufacturer && data.batchNumber) {
        return {
          ...
          details: {
            ...
            blockchainHash:
              data.blockchainHash || this.generatePrescriptionHash(data),
          },
        };
      }
  ...
  async verifyConsentOnChain(
    ...
    try {
      return await this.contract.verifyConsent(
        patientAddress,
        granteeAddress,
        consentType,
      );
  ...
  async recordSafetyAttestation(
    ...
    if (!this.contract) {
      const simulatedHash = `0x${Date.now().toString(16).padEnd(64, "0")}`;
      console.log(
        `[DEMO] Safety attestation: ${attestationHash} for prescription: ${prescriptionHash}`,
      );
      return {
        txHash: simulatedHash,
        explorerLink: `https://amoy.polygonscan.com/tx/${simulatedHash}`,
    ...
    try {
      const tx = await this.contract.recordSafetyAttestation(
        prescriptionHash,
        attestationHash,
      );
      const receipt = await tx.wait();
  ...
  async mintPrescriptionSBT(
    ...
    console.log(
      `[DEMO] SBT minted: ${simulatedTokenId} for patient: ${patientAddress}`,
    );
