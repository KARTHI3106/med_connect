...
export class AuthService {
  ...
  static async register(
    ...
    if (!name || !role || !phone || !aadhar_number) {
      return {
        success: false,
        message: "All fields are required: name, role, phone, aadhar_number",
      };
    }

    // Validate role
    if (!["patient", "doctor", "pharmacy", "caregiver"].includes(role)) {
      return {
        success: false,
        message:
          "Invalid role. Must be patient, doctor, pharmacy, or caregiver",
      };
    }

    if (role === "caregiver" && !isUsingInMemory()) {
      return {
        success: false,
        message: "Caregiver role is available in demo mode only",
      };
    }
  ...
  private static async checkPhoneExists(phone: string): Promise<boolean> {
    if (isUsingInMemory()) {
      return Array.from(inMemoryDb.users.values()).some(
        (u) => u.phone === phone,
      );
    }
    ...
    const [rows] = (await pool.execute("SELECT id FROM users WHERE phone = ?", [
      phone,
    ])) as any;
  ...
  private static async getUserByPhone(phone: string): Promise<any | null> {
    if (isUsingInMemory()) {
      return (
        Array.from(inMemoryDb.users.values()).find((u) => u.phone === phone) ||
        null
      );
    }
  ...
  private static createInMemoryProfile(
    userId: string,
    role: string,
    name: string,
  ): void {
    ...
    switch (role) {
      ...
      case "pharmacy":
        ...
        break;
      case "caregiver":
        inMemoryDb.caregivers.set(profileId, {
          id: profileId,
          user_id: userId,
          full_name: name,
  ...
  private static getInMemoryUserWithProfile(userId: string, role: string): any {
    ...
    switch (role) {
      case "patient":
        profile = Array.from(inMemoryDb.patients.values()).find(
          (p) => p.user_id === userId,
        );
        break;
      case "doctor":
        profile = Array.from(inMemoryDb.doctors.values()).find(
          (d) => d.user_id === userId,
        );
        break;
      case "pharmacy":
        profile = Array.from(inMemoryDb.pharmacies.values()).find(
          (p) => p.user_id === userId,
        );
        break;
      case "caregiver":
        profile = Array.from(inMemoryDb.caregivers.values()).find(
          (c) => c.user_id === userId,
        );
        break;
    }
  ...
  private static async getUserWithProfile(
    userId: string,
    role: string,
  ): Promise<any> {
    if (!pool) return null;
